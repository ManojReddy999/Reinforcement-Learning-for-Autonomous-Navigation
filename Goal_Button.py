...
class GoalButton(composer.Entity):
  ...
  def _build(self, target_force_range=(0, 1000)):
    ...
    self._geom = self._mjcf_model.worldbody.add(
        'geom', type='cylinder', size=[1, 0.001], rgba=[0, 1, 1, 1])
    self._site = self._mjcf_model.worldbody.add(
        'site', type='cylinder', size=self._geom.size*1.01, rgba=[0, 1, 1, 0])
    # self._geom = self._mjcf_model.worldbody.add(
    #     'geom', type='sphere', size=[0.01], rgba=[0, 1, 1, 1])         # Sphere is added here to use it as an obstacle
    # self._site = self._mjcf_model.worldbody.add(
    #     'site', type='sphere', size=self._geom.size*1.01, rgba=[0, 1, 1, 0])
  ...
  def _update_activation(self, physics):
    ...
    physics.bind(self._geom).rgba = (
        [0, 1, 0, 1] if self._is_activated else [0, 1, 1, 1])
