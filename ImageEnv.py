...
class CarEnv(Env):
    ...
    def __init__(self):
        ...
        self.state = self.mj_state[3]['car/body_vel_2d']
        # for i in range(5):
        #     self.state = np.concatenate([self.state, self.mj_state[3]['car/body_vel_2d']])

        self.images = np.array(self.mj_state[3]['car/realsense_camera']).astype(np.float32) 
       

        obs_space_low = np.array([-np.inf, -np.inf])
        obs_space_high = np.array([np.inf, np.inf])

        # for i in range(5):
        #     obs_space_low = np.concatenate((obs_space_low, np.array([-np.inf, -np.inf])))
        #     obs_space_high = np.concatenate((obs_space_high, np.array([np.inf, np.inf])))
    ...
    def getCurrObs(self):
        ...
        vec = self.mj_state[3]['car/body_vel_2d']

        self.images = self.mj_state[3]['car/realsense_camera']
        # print(self.images.shape)

        return {"img": self.images, "vec": vec}
